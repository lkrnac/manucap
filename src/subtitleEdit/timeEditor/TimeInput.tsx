...
export function validateTimeAndCursor(
    ...
    const validatedValue = showSeconds ? showMillis ? `${newH}${colon}${newM}${colon}${newS}.${newSS}` : `${newH}${colon}${newM}${colon}${newS}` : `${newH}${colon}${newM}`;
...
export default class TimeField extends React.Component<Props, State> {
    ...
    onInputChange(event: ChangeEvent<HTMLInputElement>, callback: onChangeType): void {
        ...
        const position = inputEl.selectionEnd || 1;
    ...
    render(): ReactElement {
        ...
        const onChangeHandler = (event: ChangeEvent<HTMLInputElement>) =>
            this.onInputChange(event, (v: string) => onChange && onChange(v));
